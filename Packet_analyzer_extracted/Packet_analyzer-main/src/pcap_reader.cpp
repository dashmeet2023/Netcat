...
bool PcapReader::open(const std::string& filename) {
    ...
    input_stream_->read(reinterpret_cast<char*>(&global_header_), sizeof(PcapGlobalHeader));
...
bool PcapReader::readNextPacket(RawPacket& packet) {
    ...
    input_stream_->read(reinterpret_cast<char*>(&packet.header), sizeof(PcapPacketHeader));
    if (!input_stream_->good()) {
    ...
    input_stream_->read(reinterpret_cast<char*>(packet.data.data()), packet.header.incl_len);
    if (!input_stream_->good()) {
