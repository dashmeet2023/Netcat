...
### CMake Build Instructions (Recommended)
...
2. Create and enter build directory:
   ...
   cd build
   ```
3. Generate project configuration using CMake:
...
4. Compile the project:
   ...
   ```
5. The compiled executable `dpi_engine.exe` will be located under `build\Release\` (for Visual Studio) or directly under `build\` (for MinGW). Copy it to the main `d:\Netcat` directory to activate C++ DPI acceleration.

### Build Commands Summary (Manual Compilation)
